...
- **L4ストリームプロキシ**: TCPレイヤーのロードバランシング（RoundRobin/LeastConn）、TLSパススルー、接続数制限（`l4-proxy` feature が必要）
...
- **OpenFileCache**: ファイルメタデータキャッシュ（canonicalize、metadata、mime_guessのシステムコール削減） - 静的ファイル配信で60〜67%のシステムコール削減
