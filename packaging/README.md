...
| ビルドスクリプト | `packaging/build.sh`（両方を一括生成） |
...
packaging/
├── README.md                    # 本ファイル
├── build.sh                     # 統合ビルドスクリプト（.deb + .rpm）
├── build-deb.sh                 # build.sh への後方互換ラッパー
├── Dockerfile.build             # バイナリのみ Docker ビルド
├── Dockerfile.package           # Docker 内でパッケージまで一括ビルド
├── Dockerfile.test              # Ubuntu テスト用イメージ
├── Dockerfile.test-al2023       # Amazon Linux 2023 テスト用イメージ
├── test-docker-install.sh       # .deb 検証
├── test-docker-install-rpm.sh   # .rpm 検証
├── test-install.sh              # 両方を順に検証
├── scripts/
...
./packaging/build.sh --skip-build --binary target/release/veil
```
...
2. 共通ルートファイルシステムをステージング
   ...
   - `/var/log/veil`, `/var/cache/veil`, `/var/tmp/veil`, `/var/etc/veil/ssl`
...
```bash
./packaging/build.sh          # または Docker ビルド
...
```bash
./packaging/test-docker-install-rpm.sh
```
