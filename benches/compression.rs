...
fn benchmark_compression_by_size(c: &mut Criterion) {
    ...
    for (path, size_label) in [("/", "small"), ("/large.txt", "large")].iter() {
        // 圧縮なし
        ...
        // gzip
        group.bench_with_input(BenchmarkId::new("gzip", size_label), path, |b, path| {
            b.iter(|| {
        ...
        // brotli
        group.bench_with_input(BenchmarkId::new("brotli", size_label), path, |b, path| {
            b.iter(|| {
