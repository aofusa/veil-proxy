...
`docs/artifacts/perf_reports/` に git 管理外で保持）。HPACK Huffman デコード表の再生成は `tools/gen_huffman_decode_table.py`（正本は `src/http2/hpack/huffman.rs` の `HUFFMAN_ENCODE_TABLE`、出力は `huffman_decode_table.rs` = F-121） |
