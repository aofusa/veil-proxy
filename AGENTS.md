...
| `crates/ngtcp2-sys`, `crates/nghttp3-sys` | HTTP/3 既定バックエンド用の自前 FFI。公式 [ngtcp2](https://github.com/ngtcp2/ngtcp2) / [nghttp3](https://github.com/ngtcp2/nghttp3) を cmake 静的ビルドし **aws-lc-sys** と共有リンク。`http3` feature 時、Linux/FreeBSD/macOS で `veil_http3_ngtcp2` cfg。OpenBSD/Windows または `--features http3-quiche` では `veil_http3_quiche`（Cloudflare quiche） |
| `src/http3/` | HTTP/3 サーバ。`common`（設定・圧縮）/ `stream`（アクター）/ `ngtcp2_backend` / `quiche_backend`。公開 API は `http3_server` / `http3_stream` エイリアスで後方互換 |
