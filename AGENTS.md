...
| `crates/ngtcp2-sys`, `crates/nghttp3-sys` | HTTP/3 既定バックエンド用の自前 FFI。公式 [ngtcp2](https://github.com/ngtcp2/ngtcp2) / [nghttp3](https://github.com/ngtcp2/nghttp3) を cmake 静的ビルドし **aws-lc-sys** と共有リンク。**`http3` feature は ngtcp2/nghttp3 のみ依存**（**quiche はビルドしない**）。**`http3-quiche` は quiche のみ**（**ngtcp2/nghttp3 はビルドしない**）。OpenBSD/Windows では ngtcp2/nghttp3 を target 依存から外し `http3-quiche` を必須とする |
| `src/http3/` | HTTP/3 サーバ。`common`（設定・圧縮）/ `stream`（アクター）/ `ngtcp2_backend` / `quiche_backend`。公開 API は `http3_server` / `http3_stream` エイリアスで後方互換。ngtcp2 経路はホットパス規則（非同期 I/O・バッファプール・SendFile は `runtime::io::read`/offload）に従う |
