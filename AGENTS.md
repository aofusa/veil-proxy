...
| `crates/ngtcp2-sys`, `crates/nghttp3-sys` | HTTP/3 既定バックエンド用の自前 FFI。公式 [ngtcp2](https://github.com/ngtcp2/ngtcp2) / [nghttp3](https://github.com/ngtcp2/nghttp3) を cmake 静的ビルドし **aws-lc-sys** と共有リンク。**`http3` feature は ngtcp2/nghttp3 のみ依存**（**quiche はビルドしない**）。`--features http3-quiche` 時のみ quiche を依存に追加し `veil_http3_quiche` cfg（OpenBSD/Windows 推奨） |
