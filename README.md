...
- **HTTP/2 Stream Multiplexing (concurrent in-connection processing, F-116)**: The HTTP/2 server loop uses the same **actor model** as HTTP/3: the per-connection main loop exclusively owns the `Http2Connection` (socket, HPACK state, flow-control windows, coalesce buffer) and is the **only frame reader/writer**, while every request runs as a **per-stream task** (spawned on the same-thread io_uring executor via the allocation-free `TaskPool`) doing routing/security/WASM/file/backend I/O without ever touching the connection. Tasks hand response fragments back over a bounded single-threaded SPSC channel (`H2RespMsg`: Head/Body/Trailers/Reset; bodies as `bytes::Bytes`, zero-copy) plus a wakeup `Notify` (shared primitives in `src/stream_channel.rs`); the main loop HPACK-encodes HEADERS strictly in send order, queues DATA only up to the connection/stream send windows (remainder deferred until `WINDOW_UPDATE` arrives), and coalesces frames from many streams into a single `write_buf` flush per loop iteration. Waiting is **readiness-based** — a `POLL_ADD` on the socket raced against the task `Notify` with a dependency-free two-future select — so an in-flight `read` is never cancelled and no received bytes can be lost. This removes connection-level head-of-line blocking (previously each request's full backend round-trip was awaited before reading the next frame): with `h2load -c100 -m10` the effective parallelism is 1000 streams instead of 100 connections, and one slow backend response no longer stalls other streams on the same connection
- **HTTP/3 Streaming (both directions)**: Because quiche's `Connection`/`h3::Connection` are not `Send` and must be driven by the single-threaded QUIC event loop, HTTP/3 streaming uses an **actor model** (`src/http3_stream.rs`): the main loop owns all QUIC/H3 I/O (`send_response`/`send_body`/`recv_body`) while a **backend task** (spawned on the same-thread io_uring executor) does backend TCP I/O. They communicate over a **single-threaded SPSC async channel** (`Rc<RefCell>`, **no atomics/locks**) plus a wakeup `Notify`, and the main loop multiplexes packet receive, task wakeups, and timeouts via `select_biased!`. Response bodies (`Content-Length`/chunked/EOF framing; chunked decoded zero-copy via the span decoder) and request bodies (forwarded as `Transfer-Encoding: chunked`) are carried as `bytes::Bytes` across the actor boundary without deep copies. Bounded channels give **bidirectional backpressure** (slow client → response channel full → backend reads pause; slow backend → request channel full → `recv_body` pauses → QUIC flow control throttles the client), so RSS stays bounded and does not scale with payload size. Request framing is finalized only **after the first body chunk actually arrives** (an h3 GET sends HEADERS and the fin separately, so `more_frames` alone does not imply a body). Compression-eligible responses buffer-then-compress. **TLS backends stream too** (F-44): the backend task wraps the handshaked rustls session in a full-duplex adapter (`TlsBackend`, interior mutability with borrows never held across `.await`; kTLS-migrated sessions use raw io_uring socket I/O), so upload and response run concurrently in one task just like plaintext. Eligible for `Proxy` backends over HTTP/1.1 (plaintext or TLS), `buffering` mode ≠ `full`, no WASM module, non-gRPC, security-allowed
...
**QUIC backends** (selected at compile time by `build.rs` + Cargo features):
...
- quiche (optional): <https://github.com/cloudflare/quiche>
...
```bash
# ngtcp2+nghttp3 only (quiche is NOT built or linked as a dependency)
