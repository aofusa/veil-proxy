...
SKIP_SYSCALL_CHAOS="${SKIP_SYSCALL_CHAOS:-1}"
RUN_E2E_ASAN="${RUN_E2E_ASAN:-0}"
RUN_E2E_TSAN="${RUN_E2E_TSAN:-0}"
SKIP_LIBFUZZER_ASAN="${SKIP_LIBFUZZER_ASAN:-1}"
SKIP_LIBFUZZER_TSAN="${SKIP_LIBFUZZER_TSAN:-1}"
SKIP_GITLEAKS="${SKIP_GITLEAKS:-0}"
export SKIP_SEMGREP SKIP_SBOM SKIP_ZAP SKIP_BAD_BACKEND SKIP_RESOURCE_EXHAUSTION SKIP_PUMBA SKIP_SYSCALL_CHAOS RUN_E2E_ASAN RUN_E2E_TSAN SKIP_LIBFUZZER_ASAN SKIP_LIBFUZZER_TSAN SKIP_GITLEAKS SKIP_H3SPEC
H2SPEC_FULL="${H2SPEC_FULL:-0}"
H2SPEC_STRICT="${H2SPEC_STRICT:-0}"
H3SPEC_STRICT="${H3SPEC_STRICT:-0}"
H3SPEC_REQUIRED="${H3SPEC_REQUIRED:-0}"
export H3SPEC_STRICT H3SPEC_REQUIRED
...
main() {
    ...
    # フェーズ 1d: libFuzzer + TSAN（F-71、既定 SKIP。データ競合検出）
    "${SCRIPT_DIR}/fuzz/run_libfuzzer_tsan.sh" || log "libFuzzer(TSAN) で警告（レポート参照）"
    ...
    # フェーズ 2b: h3spec（F-94。CI 必須化は F-95）
    if [[ "${SKIP_H3SPEC}" != "1" ]]; then
        run_harness h3spec h3spec || log "h3spec で警告（レポート参照）"
    else
        log "h3spec をスキップ (SKIP_H3SPEC=1)"
    fi
    ...
    "${SCRIPT_DIR}/chaos/syscall_chaos.sh" || log "syscall カオスで警告（レポート参照）"
