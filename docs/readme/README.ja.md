...
- **HTTP/3**: QUIC/UDPベースのHTTP/3（RFC 9114）。feature `http3` は **ngtcp2 + nghttp3 のみ**（公式 C ライブラリ、TLS は **aws-lc-sys** 共有）で、**quiche クレートは依存に含まない**。quiche を使う場合は `--features "http3,http3-quiche"`（OpenBSD/Windows ではこちらを推奨）
...
| `http3,http3-quiche` | 上記に **quiche を追加** | quiche |

OpenBSD/Windows では `http3,http3-quiche` を推奨。
