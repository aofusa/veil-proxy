...
- **HTTP/3**: QUIC/UDPベースのHTTP/3（RFC 9114）。Linux/FreeBSD/macOS の既定バックエンドは **ngtcp2 + nghttp3**（公式 C ライブラリ、TLS は **aws-lc-sys** 共有）。OpenBSD/Windows は **quiche** 既定。任意 OS で quiche 強制は `--features "http3,http3-quiche"`
