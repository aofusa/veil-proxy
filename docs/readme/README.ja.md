...
<p align="center">
  <img src="../images/veil_logo.webp" alt="Veil ロゴ" width="300" align="middle" />
  &nbsp;&nbsp;&nbsp;
...
- [AGENTS.md](../../AGENTS.md) — コントリビュータ・AI 向けの最小指針（運用・設計哲学・制約・参照先）。
...
- **HTTP/3**: QUIC/UDPベースのHTTP/3（RFC 9114）。feature `http3` は **ngtcp2 + nghttp3 のみ**（公式 C ライブラリ、TLS は **aws-lc-sys** 共有）で、**quiche クレートは依存に含まない**。quiche を使う場合は `--features http3-quiche` のみ（ngtcp2/nghttp3 は依存に含めない。OpenBSD/Windows 推奨）
- **高速アロケータ**: mimalloc による高速メモリ割り当て + Huge Pages対応
- **高速ルーティング**: Radix Tree (matchit) によるO(log n)パスマッチング

### プロキシ機能
- **コネクションプール**: バックエンド接続の再利用によるレイテンシ削減（HTTP/1.1・HTTPS・**H2C/HTTP-2** バックエンド対応。H2C プールはハンドシェイク済み HTTP/2 接続を gRPC/H2C リクエスト間で再利用 — F-106）
...
- **L4ストリームプロキシ**: TCPレイヤーのロードバランシング（RoundRobin/LeastConn）、TLSパススルー、`splice(2)` によるカーネル内ゼロコピー転送（ユーザースペースバッファなし）、接続数制限（`l4-proxy` feature が必要）
...
| **FreeBSD（x86_64/aarch64）** | kqueue readiness reactor | capsicum（`cap_rights_limit` / `cap_enter`）+ jail | ✗（ユーザ空間 rustls） | `[security] enable_capsicum` / `capsicum_capability_mode` / `jail_name` |
...
- `--features epoll` は Linux 専用（他ターゲットでは build.rs がエラー。BSD は kqueue が
  自動選択）。非対象 OS のセキュリティ設定キーは受理し警告して無視する。
...
|--------------|----------------|------|
| `cmake` | `http3` フィーチャー（aws-lc-sys libssl、ngtcp2/quiche と共有） | ビルド前にインストールが必要 |
...
**H2C バックエンド接続プーリング（F-106）**: H2C バックエンド接続は、HTTP/1.1・HTTPS プールと同様にリクエスト間で再利用されます（スレッドローカル `H2C_POOL`、バックエンド `host:port` をキーに管理）。同一ワーカー上の後続リクエストは、リクエストごとの TCP 接続 **および** H2C ハンドシェイク（コネクションプリフェース + SETTINGS 往復）を省略し、ストリーム ID を単調増加させながら同じ接続を再利用します。アイドル接続は `idle_connection_timeout_secs` で失効し（`max_idle_connections_per_host` で上限）、プールヒット接続が stale で初回失敗した場合は新規接続で 1 回だけ透過的にリトライします。これにより gRPC 中継（`クライアント → veil (TLS h2) → バックエンド (h2c)`）でリクエストごとのハンドシェイクを回避できます。
...
TCPレイヤー（L4）のロードバランシングプロキシです。HTTPプロキシとは異なり、プロトコルペイロードを解析せずに生のTCPストリームを転送します。データベース、メッセージブローカー、Redis、SMTP、その他の非HTTPバイナリプロトコルに適しています。
...
- **TLSパススルー**: 復号なしでTLS接続をそのまま転送
- **接続数制限**: `max_connections` を超えた接続を拒否
- **接続タイムアウト**: upstream への接続タイムアウト設定
- **ヘルスチェック連携**: L4リスナーごとにTCPまたはgRPCヘルスチェックを設定可能
...
| `tls` | TLSモード: `none`、`passthrough`、`terminate` | `none` |
| `max_connections` | 最大同時接続数（0 = 無制限） | `0` |
| `connect_timeout_secs` | upstream接続タイムアウト（秒） | `10` |
...
HTTP/3（RFC 9114）をQUIC/UDPベースでサポートします。

**QUIC バックエンド**（`build.rs` の cfg + Cargo feature）:

| feature | 依存グラフ | 実行時バックエンド |
|---------|-----------|-------------------|
| `http3` | **ngtcp2-sys + nghttp3-sys + aws-lc-sys のみ**（**quiche なし**） | ngtcp2 + nghttp3 |
| `http3-quiche` | **aws-lc-sys + quiche のみ**（**ngtcp2/nghttp3 なし**） | quiche |

OpenBSD/Windows では `http3-quiche` のみを指定（ngtcp2/nghttp3 は target 依存から除外）。
...
```bash
# ngtcp2+nghttp3 のみ（quiche は依存に含まれずビルドされない）
cargo build --release --features http3

# quiche バックエンド（依存に quiche を追加）
...
`cargo test --lib --features full` を実行 = 692 passed）で検証している。
