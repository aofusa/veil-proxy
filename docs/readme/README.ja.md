...
- **HTTP/3**: QUIC/UDPベースのHTTP/3サポート（quiche 低レベル sans-IO API: `Connection::recv`/`send`、輻輳制御・Pacing は `[http3]` で設定可能）。Linux io_uring バックエンドでは **`IORING_OP_RECVMSG` multishot + provided buffers**（F-124。reactor や multishot 不可時は POLL + `recvmmsg` にフォールバック）。0-RTT接続確立
...
| `[http3]` | `mmsg_batch_size` | `64` | UDP mmsg / io_uring multishot バッチ幅（1..=128） |
