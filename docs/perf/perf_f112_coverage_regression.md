# F-112/F-113: 網羅計測拡張後の回帰確認 + HTTP/3・gRPC ボトルネック再計測
...
`tools/perf`（F-112 で拡張したハーネス）を `CONFIG_GLOB` で scoped 実行し、各 (config, proto)
を 3 反復・median 集計。

## 目的

1. F-112 の網羅マトリクス拡張後も **HTTP/1.1・HTTP/2 に回帰がない**ことを実測で確認。
...
## HTTP/3 改善（F-113 第1段: 受信 drain バッチ）
...
| **F-113 drain 適用後** | **441.5**（440.8/442.5/441.2） | 121.8ms | 0 |
...
`recvmmsg`/`sendmmsg`（1 syscall で複数データグラム）＝ [[F-113]] 第2段。第1段は
