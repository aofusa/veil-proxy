...
| F-117 | P2 | 進行中 | [features/F-117-http2-sendfile-open-file-cache.md](features/F-117-http2-sendfile-open-file-cache.md) | HTTP/2 File 配信の open_file_cache 対応（HTTP/1.1 と経路統一）。h2_sendfile が route の open_file_cache 設定を無視・イベントループ上で同期 `Path::is_dir()` を実行（ホットパス規則違反）・canonical 封じ込め検査欠如の 3 点を `cache::get_file_info_with_config` への統一で解消。F-116 レビュー時に発見 |
