...
| F-115 | P2 | 一部完了 | [features/F-115-http3-recvmmsg-sendmmsg-batching.md](features/F-115-http3-recvmmsg-sendmmsg-batching.md) | HTTP/3 データグラムのバッチングで per-packet syscall を削減（F-111 の構造的制約に対しアプリ側 I/O バッチングで前進）。**第1段（受信 drain バッチ）実装済み**: 1 readiness あたり複数データグラムを非ブロッキング掻き出しで select/タイマー往復を償却（+2.6%・H3 E2E 117/117）。**第2段（真の recvmmsg/sendmmsg で 1 syscall 複数データグラム）は継続**。HTTP/1.1・HTTP/2・gRPC 回帰なし |
