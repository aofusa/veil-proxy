...
| F-130 | P1 | 進行中 | [features/F-130-http3-quiche-multishot-followup.md](features/F-130-http3-quiche-multishot-followup.md) | UDP/HTTP3 データプレーンの**極限 io_uring 化**（方針C）。受信 drain の io_uring 化（libc recvmmsg 排除・複数 RECVMSG in-flight）→ 真 `IORING_RECV_MULTISHOT`+buffer ring → 送信 `IORING_OP_SENDMSG`+GSO cmsg（libc sendmmsg 排除）。quiche は sans-IO のまま。各フェーズ perf A/B・AGENTS ホットパス厳守（旧番号衝突のため F-125→F-130 へ改番） |
