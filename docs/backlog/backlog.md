...
| B-46 | P1 | 未着手 | [bugs/B-46-http3-buffered-proxy-body-stall.md](bugs/B-46-http3-buffered-proxy-body-stall.md) | HTTP/3 + buffering full の Proxy 応答で 2xx ヘッダのみ届きボディ 0B・全ストリームエラー（B-43 と同型のヘッダ/ボディ順序スタックがバッファ経路で再発の疑い）。streaming 経路・H1/H2 は正常。F-118 perf 計測で検出 |
