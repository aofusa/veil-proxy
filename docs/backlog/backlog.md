...
| F-18 | P1 | 完了 | [features/l4-stream-proxy.md](features/l4-stream-proxy.md) | L4 (TCP/UDP) ストリームプロキシ。UDP 対応は F-124 で実装 |
...
| F-58 | P1 | 完了 | [features/F-58-perf-report-glibc-musl-nginx.md](features/F-58-perf-report-glibc-musl-nginx.md) | パフォーマンス測定レポート（glibc/musl/nginx 比較・B-13/B-14/B-15 修正後の再測定）。全 24 計測 Non-2xx=0、要約は `docs/perf/README.md`（計測履歴） |
...
| F-107 | P1 | 完了 | [features/F-107-http3-grpc-streaming-web-coverage.md](features/F-107-http3-grpc-streaming-web-coverage.md) | レポート残件: H3 gRPC ストリーミング細分化 3 + 圧縮/encoding 5 + gRPC-Web 3 + container_security S-G-H3-14〜17。**CI は F-108** |
| F-108 | P2 | 未着手 | [features/F-108-http3-grpc-streaming-web-ci.md](features/F-108-http3-grpc-streaming-web-ci.md) | F-107 追加 E2E/プローブの CI 組み込み（F-107 から分離。**本作業では対象外**） |
| F-109 | P1 | 完了 | [features/F-109-http3-grpc-parity-coverage.md](features/F-109-http3-grpc-parity-coverage.md) | http3_grpc_test_coverage_report 最新版: H3 パリティ E2E 9 + container_security 4 系統（smuggling/WS/feat/grpc-web）。**CI は F-110** |
| F-110 | P2 | 未着手 | [features/F-110-http3-grpc-parity-ci.md](features/F-110-http3-grpc-parity-ci.md) | F-109 追加 E2E/プローブの CI 組み込み（F-109 から分離。**本作業では対象外**） |
| F-111 | P2 | 完了 | [features/F-111-perf-regression-check-http3-gso-zerocopy.md](features/F-111-perf-regression-check-http3-gso-zerocopy.md) | perf 回帰確認（HTTP/1.1・HTTP/2・gRPC(F-106) は退行なしを実測で確認）+ HTTP/3 送信 GSO バッチのゼロコピー化（二重確保・二重コピー排除）。HTTP/3 の構造的な速度差（HTTP/2 比 約 1/6.3）は userspace QUIC の per-packet syscall コストに起因し、既存の調査結論（F-89/F-105/F-106）を追認 |
| F-112 | P1 | 完了 | [features/F-112-http3-grpc-cache-lb-coverage.md](features/F-112-http3-grpc-cache-lb-coverage.md) | http3_grpc_test_coverage_report 最新版: H3 キャッシュ/LB/ヘルス/413/431/圧縮/smuggling E2E + gRPC LC/ヘルス/max streams/CORS + fuzz qpack/h3 + pumba H3。**CI は F-113** |
| F-113 | P2 | 未着手 | [features/F-113-http3-grpc-cache-lb-ci.md](features/F-113-http3-grpc-cache-lb-ci.md) | F-112 追加 E2E/プローブの CI 組み込み（F-112 から分離。**本作業では対象外**） |
| F-114 | P2 | 完了 | [features/F-114-perf-coverage-all-protocol-feature-matrix.md](features/F-114-perf-coverage-all-protocol-feature-matrix.md) | perf ハーネスの全プロトコル×全機能 網羅計測（`docs/artifacts/perf_coverage_report.md` 起点。ID は先行の F-112/F-113 と衝突回避のため採番）。gen_configs.sh をヘルパー関数化しグループA(H1/H2 Proxy+feat)/B(H3 File・Proxy+feat)/C(gRPC H2/H3+feat) を規則的命名で生成、run_perf.sh のディスパッチを汎用化。gRPC over H3 はクライアント非対応でフェイルセーフ NA、L4+L7機能は設定非対応で N/A。既存16直交表+nginxベースライン互換維持・`CONFIG_GLOB` で scoped 計測。回帰確認済み（`docs/perf/README.md`） |
| F-115 | P2 | 完了 | [features/F-115-http3-recvmmsg-sendmmsg-batching.md](features/F-115-http3-recvmmsg-sendmmsg-batching.md) | HTTP/3 データグラムのバッチングで per-packet syscall を削減。**第1段（受信 drain バッチ）**: select/タイマー往復を償却（+2.6%）。**第2段（真の recvmmsg/sendmmsg で 1 syscall 最大 16 データグラム、送信は sweep 全体を sendmmsg 一括送出）実装済み**（+5.2%、Docker seccomp へ recvmmsg/sendmmsg 追加必須）。A/B の過程で **B-43**（HTTP/3 低スループットの真の主因）を発見・修正し合計 **421→853 req/s（+103%、HTTP/2 比 16%→32%）**。HTTP/1.1・HTTP/2・gRPC 回帰なし・H3 E2E 117/117。詳細 `docs/perf/README.md` |
| F-116 | P1 | 完了 | [features/F-116-http2-stream-multiplexing.md](features/F-116-http2-stream-multiplexing.md) | HTTP/2 ストリーム多重化（コネクション内並行処理）。`handle_http2_requests` がリクエスト成立ごとにバックエンド往復を `await` する直列処理（アプリ層 HoL Blocking）を、HTTP/3 実証済みの単一スレッド・アクターモデル（per-stream タスク + 有界チャネル + Notify + `POLL_ADD` readiness 待ち）へ移植して解消。**A/B 実測 HTTP/2 +16.1%（3646 req/s、HTTP/1.1 超え・nginx 比 1.47 倍）、HTTP/1.1 非劣化**（h2load 既定 1 スレッドのクライアント律速に注意、`docs/perf/README.md`）。アップロードの recv ウィンドウ補充を消費連動化（バックプレッシャ維持）、多重化 E2E 追加、E2E 530/530。起点調査: `docs/artifacts/h2_performance_analysis.md`、設計: `docs/artifacts/h2_multiplexing_design.md` |
| F-117 | P2 | 完了 | [features/F-117-http2-sendfile-open-file-cache.md](features/F-117-http2-sendfile-open-file-cache.md) | HTTP/2 File 配信の open_file_cache 対応（HTTP/1.1 と経路統一）。h2_sendfile が route の open_file_cache 設定を無視・イベントループ上で同期 `Path::is_dir()` を実行（ホットパス規則違反）・canonical 封じ込め検査欠如の 3 点を `cache::get_file_info_with_config` への統一で解消。F-116 レビュー時に発見 |
| F-118 | P2 | 完了 | [features/F-118-perf-harness-l4-ready-tsv-order.md](features/F-118-perf-harness-l4-ready-tsv-order.md) | perf ハーネス改善（`docs/artifacts/perf_improvement_plan.md` 起点）。`wait_ready` の L4（平文 9080）readiness 確認・TSV 出力順（nginx → glibc → musl）の明文化。nginx ベースラインは現状維持（http1.1/http2 のみ）・gRPC over H1 は対象外。改修後にフルスイート実測（全 105 構成×プロトコル Non-2xx=0）し docs/perf を v0.5.0 向けに最新化。計測起点で B-44/B-45/B-46 を検出・修正 |
| F-119 | P2 | 完了 | [features/F-119-ci-skip-e2e-job.md](features/F-119-ci-skip-e2e-job.md) | CI の `e2e` ジョブをスキップ化（v0.5.0 リリース準備。E2E はローカル `./tests/e2e_setup.sh test` 運用、ジョブ定義は `if: false` で残す） |
| F-121 | P1 | 完了 | [features/F-121-hpack-huffman-lut-decode.md](features/F-121-hpack-huffman-lut-decode.md) | HPACK Huffman デコードの **4-bit LUT**（テーブル駆動状態遷移）化。256 状態×16 peek・16 KiB L1 常駐。線形探索を廃し release マイクロベンチ **~11.9×**。B-21 / I3–I6 パディング維持。tools/perf HTTP/2 scoped で相対改善を確認（設計: `docs/artifacts/hpack_huffman_lut_design.md`） |
| F-122 | P2 | 完了 | [features/F-122-openbsd-tls-crypto-provider.md](features/F-122-openbsd-tls-crypto-provider.md) | OpenBSD TLS 解決。rustls の暗号プロバイダを OpenBSD 限定で ring へ切替（aws-lc-rs は OpenBSD でハンドシェイク未完）。Cargo は target 別依存 + resolver v2 で aws-lc を完全除外。unveil の exists() 事前確認バグ（2番目以降が未 unveil→404）も修正。静的/プロキシとも HTTPS 200、pledge+unveil 有効で動作 |
| F-123 | P3 | 完了 | [features/F-123-capsicum-capability-mode-openat.md](features/F-123-capsicum-capability-mode-openat.md) | FreeBSD capability mode（cap_enter）での静的ファイル配信対応（ルートディレクトリ fd + openat/fstatat + O_RESOLVE_BENEATH/AT_RESOLVE_BENEATH 相対化）を実装。cap_enter 前に File ルート dirfd を確保し配信 open/stat を相対化、OpenOptions 単一チョークポイント化。cap-mode 非互換の clock_nanosleep(std sleep) は select(2) ベース cap_safe_sleep へ置換。FreeBSD 14.3 VM E2E で index/nested 200・トラバーサル 403・panic0 を確認 |
| F-126 | P2 | 完了 | [features/F-126-freebsd-ktls.md](features/F-126-freebsd-ktls.md) | FreeBSD kTLS 送受信オフロード対応（v0.6.0）。`TCP_TXTLS_ENABLE`/`TCP_RXTLS_ENABLE` を新規 `src/ktls_freebsd.rs` に隔離実装し Linux 経路（`src/ktls.rs`/`src/ktls_rustls.rs`）は無変更。既存 `ktls` feature を流用、build.rs の `veil_ktls` cfg を freebsd にも拡張。`struct tls_enable` の ABI は FreeBSD 14.3 実機ヘッダで確認（`CRYPTO_AES_NIST_GCM_16` は実機値 25）。FreeBSD VM 実機で kTLS 有効化・フォールバック・複数レコードボディを確認。設計 `docs/artifacts/f126_freebsd_ktls_design.md` |
| F-127 | P2 | 完了 | [features/F-127-freebsd-aio.md](features/F-127-freebsd-aio.md) | FreeBSD kqueue AIO（POSIX AIO）実装（v0.6.0）。`--features aio`（`veil_aio`、FreeBSD 専用・既定オフ）で `TcpStream::read`/`write` を `aio_read(2)`/`aio_write(2)` + `EVFILT_AIO` 完了通知ベースへ切替。新規 `src/runtime/reactor/aio.rs`（uring 版 `OpTable` と同型のスラブ + token 世代管理、`Box<aiocb>` で固定アドレス生存を保証、Future drop 時は `aio_cancel` + Detached 状態で完了 kevent 到着まで待つ orphan 回収）。`EAGAIN` は per-I/O で readiness へフォールバック。既存 kqueue readiness 経路・Linux io_uring は無改変。設計 `docs/artifacts/f127_freebsd_aio_design.md` |
| F-124 | P2 | 完了 | [features/F-124-l4-udp-proxy.md](features/F-124-l4-udp-proxy.md) | L4 UDP プロキシ（F-18 の TCP のみだった構成に UDP を追加）。セッションテーブル方式（クライアントアドレスをキーにした疑似セッション、`idle_timeout_secs` で自動退去）。`runtime/udp.rs` に汎用非同期 UDP ソケットを新設（io_uring/reactor 両バックエンド共通、新規 io_uring オペコード追加なし）。UDP は DTLS 非対応のため `tls` 指定は警告して無視 |
| F-129 | P1 | 完了 | [features/F-129-http3-quiche-cc-multishot.md](features/F-129-http3-quiche-cc-multishot.md) | HTTP/3: quiche 輻輳制御/Pacing/HyStart と mmsg バッチ（既定 64）を `[http3]` 設定化。受信先頭を `IORING_OP_RECVMSG`+POLL_FIRST 単発 + recvmmsg drain へ（`feat/http3-quiche`、旧番号衝突のため F-124→F-129 へ改番）。io_uring restriction に RECVMSG/PROVIDE_BUFFERS を追加。真 multishot は F-130 で完成 |
| F-130 | P1 | 完了（C1+C3、C2は次段） | [features/F-130-http3-quiche-multishot-followup.md](features/F-130-http3-quiche-multishot-followup.md) | UDP/HTTP3 データプレーンの**極限 io_uring 化**（方針C フェーズ1+ 達成）。**C1 受信 drain の io_uring 化**（libc recvmmsg 排除・N 本の独立 `IORING_OP_RECVMSG` を常時 in-flight・per-slot 固定 msghdr で peer 安全・ホットパス alloc ゼロ）+ **C3 送信 io_uring 化**（`IORING_OP_SENDMSG`+GSO `UDP_SEGMENT` cmsg・libc sendmmsg 排除・複数 SQE を1回submit）。quiche は sans-IO のまま。host-net h2load HTTP/3 back-to-back A/B で **F-129 比 +5.2%**（median 6955→7318 req/s・全2xx）。真 `IORING_RECV_MULTISHOT`+buffer ring（C2）は kernel6.0+ 依存・multi-peer 安全性コスト・F-129 不安定化実績から次段送り（フォールバック維持）。設計 `docs/artifacts/f130_udp_iouring_design.md`（旧番号衝突のため F-125→F-130 へ改番） |
| F-120 | P1 | 完了 | [features/F-120-cross-platform-epoll-kqueue-bsd.md](features/F-120-cross-platform-epoll-kqueue-bsd.md) | クロスプラットフォーム対応。runtime をコンパイル時バックエンド分離（デフォルト io_uring 不変・性能非劣化）し、Linux `--features epoll` フォールバック、aarch64-linux クロスビルド（Dockerfile + QEMU 検証）、FreeBSD（kqueue + capsicum + jail）、OpenBSD（kqueue + pledge + unveil、kTLS 非対応）へ対応。seccomp はバックエンド別に最小権限分割。packaging も対象ターゲット拡張。設計 `docs/artifacts/f120_cross_platform_design.md` |
| F-125 | P2 | 完了（macOS+Windows） | [features/F-125-windows-macos.md](features/F-125-windows-macos.md) | macOS（universal2-apple-darwin）対応。既存 kqueue reactor を再利用し、accept4/MSG_NOSIGNAL/pipe2/SOCK_NONBLOCK 非搭載への cfg 適応 + ネイティブセキュリティ `sandbox_init`（Seatbelt、保守的な deny-default + 書き込みのみ制限プロファイル）を実装。TLS 暗号は **ring** プロバイダ（aws-lc-sys の手書きアセンブリが zig でクロスリンク不可・release で NO_ASM 禁止のため。OpenBSD と同じ ring 経路を macOS へ拡張）。`messense/cargo-zigbuild` で universal2 Docker クロスビルド成功（http3/wasm 除く feature セット）。Linux 無回帰確認済み。**Windows（x86_64-pc-windows-msvc）は v0.6.0 で同チケット継続作業として完了**（WSAPoll reactor + Winsock ソケット層 + Job Object セキュリティ、`cargo xwin build` クロスビルド、TLS は ring）。aarch64-pc-windows-msvc は ring の prebuilt asm 非対応のため aws_lc_rs でクロスビルド対応。QEMU・実機検証は Windows/macOS とも未実施。設計 `docs/artifacts/f125_windows_macos_design.md` |
| F-131 | P1 | 進行中 | [features/F-131-cross-platform-ssl-udp.md](features/F-131-cross-platform-ssl-udp.md) | クロスプラットフォーム TLS (aws-lc-rs)・Windows UDP モジュール・BoringSSL quiche 対応。macOS / Windows x86_64 の TLS 暗号プロバイダを aws-lc-rs へ統一し、quiche の暗号バックエンドを boringssl へ切り替え、Windows UDP 非同期ソケット層を実装して全ターゲット full features ビルド対応。 |
...
| F-89 | P1 | 完了 | [features/F-89-perf-full-features-coverage.md](features/F-89-perf-full-features-coverage.md) | perf ハーネスの full features 網羅拡充（`docs/perf/README.md` 計測履歴 §6 起点）。wasm/metrics/access-log/rate-limit/admin/otel/l4 の feat 構成 9 種 + パススルー WASM モジュールを追加し計測（全 Non-2xx=0）。**TLS 終端が支配的・L7 機能ロジックはノイズ範囲内**と判明し、WASM ライフサイクルの重複エクスポート解決を排除。第 3 弾で **http3(QUIC h2load)/grpc(k6→grpcbin)/websocket(k6→echo-server)** の 3 構成を専用クライアントで追加し計測（合計 30 構成、Non-2xx=0）。gRPC h2c 中継バグは B-40(F-92) 修正済みを本ハーネスで再検証 |
...
| B-44 | P1 | 完了 | [bugs/B-44-h2-proxy-backend-conn-churn-port-exhaustion.md](bugs/B-44-h2-proxy-backend-conn-churn-port-exhaustion.md) | HTTP/2 プロキシが F-116 多重化後にバックエンド接続チャーン（プールのアイドル上限 8 << 同時ストリーム ~250/スレッド）でエフェメラルポート枯渇（EADDRNOTAVAIL → 502 混入・~590 req/s へ −73% 退行）。4 段修正（プール上限 8→256 / EADDRNOTAVAIL バックオフリトライ / ホスト単位 connect 並行数ゲート / 起動時 RLIMIT_NOFILE 自動引き上げ=真因）で解消。修正後は全構成 Non-2xx=0・HTTP/2 プロキシ ~1950 req/s に回復。F-118 perf 計測で検出 |
| B-46 | P1 | 完了 | [bugs/B-46-http3-buffered-proxy-body-stall.md](bugs/B-46-http3-buffered-proxy-body-stall.md) | HTTP/3 + buffering full の Proxy 応答で 2xx ヘッダのみ届きボディ 0B・全ストリームエラー。真因は send_response の無条件 content-length 付与とバックエンド由来 content-length の重複（nghttp3 が H3_MESSAGE_ERROR で拒否）。重複時は付与しない修正 + ボディ内容一致 E2E を追加し、scoped 再計測で http3 ~600 req/s・エラー 0 に回復。F-118 perf 計測で検出 |
| B-45 | P1 | 完了 | [bugs/B-45-l4-half-close-fd-exhaustion.md](bugs/B-45-l4-half-close-fd-exhaustion.md) | L4 プロキシが片方向 EOF 時に `shutdown(SHUT_WR)` を対向へ伝搬せず、クローズ済み接続が 4 fd（ソケット2 + パイプ2）をアイドルタイムアウトまで滞留 → 接続チャーンで EMFILE → accept/connect 全滅（wrk 3 回目で 0 req/s）。転送ループ離脱時の半クローズ伝搬（shutdown SHUT_WR）で修正し、wrk 3 連続 ~5000 req/s 安定を確認。F-118 perf 計測で検出 |
