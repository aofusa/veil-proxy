...
| F-116 | P1 | 進行中 | [features/F-116-http2-stream-multiplexing.md](features/F-116-http2-stream-multiplexing.md) | HTTP/2 ストリーム多重化（コネクション内並行処理）。`handle_http2_requests` がリクエスト成立ごとにバックエンド往復を `await` する直列処理（アプリ層 HoL Blocking）を、HTTP/3 実証済みの単一スレッド・アクターモデル（per-stream タスク + 有界チャネル + Notify + `POLL_ADD` readiness 待ち）へ移植して解消。起点調査: `docs/artifacts/h2_performance_analysis.md`、設計: `docs/artifacts/h2_multiplexing_design.md` |
