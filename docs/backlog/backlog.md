...
| F-32 | P1 | 一部完了 | [features/F-32-http2-http3-streaming-body.md](features/F-32-http2-http3-streaming-body.md) | HTTP/2 レスポンス+リクエスト方向の真のストリーミング実装。第1: 非圧縮+content-length+非chunked を DATA 逐次転送。第2: chunked を `ChunkedDecoder::next_data_span` でゼロコピー逐次デコード。第3: リクエスト方向（HEADERS 受信時にバックエンド接続開始、DATA を chunked でゼロコピー逐次転送、`recv_data_for_streaming` で非バッファ・フロー制御、書込完了までバックプレッシャ）。付随で rustls received_plaintext 16KB 上限超過バグ（大容量 h2/TLS アップロード）を修正。HTTP/3 は継続 |
