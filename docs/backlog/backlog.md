...
| F-90 | P1 | 完了 | [features/F-90-container-security-full-features.md](features/F-90-container-security-full-features.md) | container_security full features 網羅（14 プローブ + http3/ws クライアント + テストケース文書 + 実行検証）。実装バグは B-29〜B-34 へ起票 |
...
| B-31 | P2 | 完了 | [bugs/B-31-rate-limit-thread-local-per-worker.md](bugs/B-31-rate-limit-thread-local-per-worker.md) | レートリミットが thread_local でワーカー分散。**修正済み**: `GLOBAL_RATE_LIMITER`（Mutex）へ集約。F-90 rate_limit_probe 通過 |
