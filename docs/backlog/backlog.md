...
| F-121 | P1 | 進行中 | [features/F-121-hpack-huffman-lut-decode.md](features/F-121-hpack-huffman-lut-decode.md) | HPACK Huffman デコードの 8bit LUT（テーブル駆動状態遷移）化。`HUFFMAN_ENCODE_TABLE` 線形探索を廃し L1 常駐テーブルで HEADERS ホットパスの分岐/キャッシュミスを削減。B-21 ガード維持・perf HTTP/2 で改善確認 |
