...
| F-123 | P3 | 未着手 | [features/F-123-capsicum-capability-mode-openat.md](features/F-123-capsicum-capability-mode-openat.md) | FreeBSD capability mode（cap_enter）での静的ファイル配信対応（ルートディレクトリ fd + openat/O_RESOLVE_BENEATH 相対化）。現状 capability mode ではパス指定 open が禁止され fail-closed 404（F-120 Phase 4 起点） |
