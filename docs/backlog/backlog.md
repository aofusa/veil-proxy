...
| F-118 | P2 | 進行中 | [features/F-118-perf-harness-l4-ready-tsv-order.md](features/F-118-perf-harness-l4-ready-tsv-order.md) | perf ハーネス改善（`docs/artifacts/perf_improvement_plan.md` 起点）。`wait_ready` の L4（平文 9080）readiness 確認・TSV 出力順（nginx → glibc → musl）の明文化。nginx ベースラインは現状維持（http1.1/http2 のみ）・gRPC over H1 は対象外。改修後にフルスイート実測し docs/perf を v0.5.0 向けに最新化 |
| F-119 | P2 | 進行中 | [features/F-119-ci-skip-e2e-job.md](features/F-119-ci-skip-e2e-job.md) | CI の `e2e` ジョブをスキップ化（v0.5.0 リリース準備。E2E はローカル `./tests/e2e_setup.sh test` 運用、ジョブ定義は `if: false` で残す） |
