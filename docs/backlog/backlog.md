...
| F-112 | P2 | 進行中 | [features/F-112-perf-coverage-all-protocol-feature-matrix.md](features/F-112-perf-coverage-all-protocol-feature-matrix.md) | perf ハーネスの全プロトコル×全機能 網羅計測（`docs/artifacts/perf_coverage_report.md` 起点）。gen_configs.sh をヘルパー関数化しグループA(H1/H2 Proxy+feat)/B(H3 File・Proxy+feat)/C(gRPC H2/H3+feat) を規則的命名で生成、run_perf.sh のディスパッチを汎用化。gRPC over H3 はクライアント非対応でフェイルセーフ NA、L4+L7機能は設定非対応で N/A。既存16直交表+nginxベースライン互換維持・`CONFIG_GLOB` で scoped 計測 |
| F-113 | P2 | 進行中 | [features/F-113-http3-recvmmsg-sendmmsg-batching.md](features/F-113-http3-recvmmsg-sendmmsg-batching.md) | HTTP/3 データグラムの recvmmsg/sendmmsg バッチングで per-packet syscall を削減（F-111 の構造的制約に対しアプリ側 I/O バッチングで前進）。異なる接続のデータグラムも 1 syscall にまとめ Docker veth（GSO/GRO 無効）でも syscall 回数を削減。HTTP/1.1・HTTP/2・gRPC 回帰なし・H3 E2E グリーンが受け入れ条件 |
