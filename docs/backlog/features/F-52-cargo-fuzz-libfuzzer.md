...
新ターゲット `http_request_smuggling` をネイティブ実行:

| ターゲット | 結果 |
|-----------|------|
| `http_request_smuggling` | **258,976 runs / ~6,166 exec/s、peak RSS 152MB、クラッシュなし**。反デシンク不変条件（CL+TE は必ず拒否）が全入力で成立（assert 失敗なし） |
