# F-115: HTTP/3 データグラム recvmmsg/sendmmsg バッチングによる per-packet syscall 削減
...
## 実測知見（F-114 計測 2026-07-12・`docs/perf/perf_f114_coverage_regression.md`）
...
### 第2段（実装済み・2026-07-13）: 真の recvmmsg/sendmmsg バッチング

- **受信**: 主ループ drain を `recvmmsg(2)`（MSG_DONTWAIT、1 syscall で最大
  `MMSG_RECV_BATCH=16` データグラム、異なるフローも同時）へ置換。セグメント処理は
  `process_datagram_segments` へ抽出し select 経由の初弾と共用（`src/udp/socket.rs` の
  `MmsgRecvScratch`/`recv_mmsg_sync`）。
- **送信**: `send_pending_packets` を per-connection 即時 flush から sweep 全体の送信
  エントリ蓄積 → `sendmmsg(2)` 一括送出へ変更（`MMSG_SEND_BATCH=16` / 256KB 閾値で途中送出、
  GSO 無効時はパケット境界へ展開。`MmsgSendScratch`/`send_mmsg_async`）。
- mmsghdr/iovec/sockaddr/cmsg は Box 固定アドレスのスクラッチ再利用で per-sweep 確保なし。
  EAGAIN は既存 POLL_ADD 待機のみ（新規 io_uring オペコードなし）。
- **`docker/assets/security/seccomp.json` に `recvmmsg`/`sendmmsg` を追加**（欠けると
  defaultAction=ERRNO の EPERM で HTTP/3 が 0 req/s。glibc で顕在化・musl は libc
  フォールバックで動作）。veil 内蔵 seccomp は従来から許可済み。
- 効果: 第2段単体で 421→443 req/s（+5.2%）。A/B の過程で **B-43**（StreamBlocked 後の
  ヘッダ未送出ストリームが FrameUnexpected で永久スタック = HTTP/3 低スループットの主因）を
  発見・修正し、合計で **421→853 req/s（+103%、HTTP/2 比 16%→32%）**・h2load failed=0。
  詳細: `docs/perf/perf_f115_stage2_b43.md`。受け入れ条件（H1/H2/gRPC 回帰なし・HTTP/3 E2E
  全通過・新規 io_uring オペコードなし）をすべて満たし **完了**。
