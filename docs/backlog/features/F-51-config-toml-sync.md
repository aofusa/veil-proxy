...
- 全 `src/**/*.rs` の Deserialize 構造体フィールドと config.toml のキー（コメント例示含む）を機械的に突き合わせて同期。
...
- **回帰防止**: 同梱 config.toml がパース・バリデーション可能であることを保証する単体テスト `shipped_config_toml_parses_and_validates`（`veil -t` 相当、プレースホルダーパスのみ差し替え）を追加。
- README.md / README.ja.md の `[tls]` 設定表に `cipher_suites` / `auto_reload` / `reload_interval_secs` を追記。
- **出典**: ユーザー要求
