...
## 対応状況: 完了（HTTP/2 レスポンス方向 content-length + chunked / リクエスト方向 / HTTP/3 レスポンス+リクエスト方向すべてストリーミング化）
...
## 対応状況（全面書換・第4フェーズ）: HTTP/3 レスポンス+リクエスト方向の真のストリーミング

quiche の `Connection`/`h3::Connection` は `Send` でなく単一スレッドのメインループが UDP I/O・
ACK・フロー制御を専有駆動するため、バックエンド TCP I/O をメインループ内で同期的に行うと QUIC
コネクション全体が停止する。そこで **アクターモデル**でレスポンス/リクエスト双方向の全
バッファリングを排除した。

### 実装（`src/http3_stream.rs` 新規 + `src/http3_server.rs`）

- **アクターモデル**: メインループ（QUIC/H3 アクター）が `conn.send/recv`・`h3.poll/send_response/
  send_body/recv_body` を唯一駆動し、**バックエンドタスク**（`crate::runtime::spawn` で同一スレッド
  executor 上に起動、io_uring `TcpStream`）がバックエンド TCP I/O を担う。両者を **単一スレッド
  SPSC 非同期チャネル**（`Rc<RefCell>` ベース、**アトミック・ロックなし**）+ `H3Notify` で接続する。
- **レスポンス方向**: バックエンドタスクが応答 head→body を逐次受信し `RespMsg` でメインループへ。
  length/chunked/EOF フレーミングに対応し、chunked は `ChunkedDecoder::next_data_span` でゼロコピー
  デコード。メインループは `send_body` でクライアントへ。**圧縮対象時のみ**全バッファ後に圧縮
  （HTTP/2 第1フェーズと同方針）。
- **リクエスト方向**: メインループが `recv_body` でボディ断片を取り出し（`bytes::Bytes` ゼロコピー）、
  req チャネル経由でタスクへ。タスクは chunked エンコードでバックエンドへ逐次転送。**最初の
  ボディ断片が実際に届くかを peek してから framing を確定**する（h3 クライアントは HEADERS と fin を
  別送するためボディのない GET でも `more_frames=true`。これを誤って chunked 化するとボディを読まない
  バックエンドで RST する不具合を回避）。`max_request_body_size` を転送中に強制（超過時 413 + リセット）。
- **双方向バックプレッシャ**: 有界チャネル（容量 8）で、クライアント遅延 → resp チャネル満杯 →
  バックエンド read 停止、バックエンド遅延 → req チャネル満杯 → `recv_body` 停止 → QUIC フロー制御で
  クライアント送信停止、が自然伝播。プロセスのヒープ保持は「並行ストリーム数 × 有界バッファ」に
  収まり **RSS が総ペイロードサイズに比例しない**。
- **メインループ**: `futures::select_biased!`(recv_gro / notify / sleep) で多重化し、毎イテレーションで
  全ハンドラの `drive_proxy_streams`（req pump + resp flush）を駆動。バックエンドタスクの notify で
  低遅延に駆動する（負け arm の `recv_gro_async` drop は既存 timeout と同じく cancel-safe）。
- **適格性**: Proxy バックエンド + バッファリング非 Full + 非 gRPC + WASM モジュール非適用 +
  平文バックエンド（TLS は当面バッファ経路へフォールバック）+ セキュリティ許可。非適格は既存の
  `handle_request`（バッファ経路）で処理（回帰なし）。セキュリティ拒否はアップロードを溜めないよう即時拒否。

### 付随修正（E2E で発覚）

- **EAGAIN ビジースピン**: io_uring SEND/RECV の EAGAIN 時に `writable()`/`readable()`（POLL_ADD）で
  待ってからリトライする方式へ変更（`read_backend`/`write_all`）。従来は即リトライしてイベント
  ループへ制御を返さず、大容量転送でメインループ（QUIC 駆動）が starve しストールしていた。

### 検証（第4フェーズ）

- 単体: `src/http3_stream.rs` のチャネル送受信/容量/EOF/Closed/async・Notify・chunk size 行・
  ヘッダ終端/ステータス/レスポンスヘッダパースの 11 件。`cargo test --bins --features full` 612 通過。
- 結合: `cargo test --test integration_tests --features full` 53/53 通過。
- E2E: `test_http3_request_body_streaming`（1,200,000 バイト、初期ストリームウィンドウ 1MB 超過の
  往復バイト完全一致）、`test_http3_request_body_streaming_small`（単一 DATA 経路）、
  `test_http3_response_body_streaming`（chunked バックエンド 200,000 バイトをバイト完全一致で逐次
  デコード転送）を追加。`features full` の HTTP/3 E2E 33/33 通過。フルスイートは 395/397（残り 2 件は
  `test_concurrent_connection_stress`（HTTPS のみ）/ `431` の既知の負荷フレーキーで co-tenant CPU 負荷
  由来・コード非依存。単独/低負荷で通過）。

### 残

- **TLS バックエンドのストリーミング**: 現状 TLS バックエンドはバッファ経路（既存のブロッキング TLS）に
  フォールバック。async TLS（io_uring 上の rustls/kTLS）との結合は独立タスクとして継続。
