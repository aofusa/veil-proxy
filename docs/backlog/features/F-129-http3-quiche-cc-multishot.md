# F-129: HTTP/3 quiche 輻輳制御・バッチ設定 + IORING_OP_RECVMSG 受信経路

**ステータス: 完了（2026-07-19）— ただし真 Multishot は未達。後続は [F-130](F-130-http3-quiche-multishot-followup.md)**
...
- `src/runtime/uring/executor.rs` / `ring.rs`（RECVMSG 許可。multishot OpTable は残骸として残存 → F-130）
- `src/http3_server.rs`（CC 適用・受信分岐）
- `examples/config.toml` / `contrib/config/config.toml` / README / README.ja

## 受け入れ条件（F-129 時点で満たしたもの）
...
- [ ] ~~真 Multishot~~ → **F-130**
- [ ] ~~AGENTS.md / コメント整合 / tools/perf 公式~~ → **F-130**
...
- 後続: [F-130](F-130-http3-quiche-multishot-followup.md)（**委譲用フル引き継ぎドキュメント**）
