...
完了（fix/test）。  
詳細: `docs/artifacts/f107_failed_tests_report.md`  
検証: 単体 725 / 統合 53 / E2E 501 PASS（F-107 新規 11） / feature マトリクス warning 0。  
container_security: `veil:glibc` 再ビルド（Docker 内単体 725）+ `run.sh` exit 0、S-G-H3-14〜17 PASS。  
付随修正: メトリクス単体の並列トグル偽失敗を Mutex 直列化で解消。