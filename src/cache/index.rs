...
impl CacheIndex {
    ...
    pub fn insert(&self, key: CacheKey, entry: CacheEntry) {
        ...
        if let Some(old) = self.entries.insert(hash, index_entry) {
            ...
            if memory > old_memory {
                self.memory_usage
                    .fetch_add(memory - old_memory, Ordering::Relaxed);
            } else {
                self.memory_usage
                    .fetch_sub(old_memory - memory, Ordering::Relaxed);
            }
        } else {
    ...
    pub fn remove(&self, key: &CacheKey) -> Option<Arc<CacheEntry>> {
        ...
        if let Some((_, removed)) = self.entries.remove(&hash) {
            ...
            if removed.key == *key {
                ...
                self.memory_usage
                    .fetch_sub(removed.entry.memory_usage(), Ordering::Relaxed);
    ...
    pub fn evict_lru(&self, max_memory: usize) -> usize {
        ...
        let target_free = current_memory.saturating_sub(max_memory) + (max_memory / 10); // 10%余分に解放
